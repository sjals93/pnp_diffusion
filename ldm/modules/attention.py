...
#         # pnp diffusion + scaled self-attention
#         if self.ratio_for_self_att >= 0.0 and self.ratio_for_self_att < 1.0:
...
#                 q_uncond, q_cond = q_injected.chunk(2)
#                 q_sa = torch.cat([q_uncond] * b + [q_cond] * b, dim=0)
#                 q = self.ratio_for_self_att * q_sa + (1. - self.ratio_for_self_att) * q

#             # k
#             k = self.to_k(context)
#             k = rearrange(k, 'b m (h d) -> (b h) m d', h=h)

#             if k_injected is not None:

#                 k_uncond, k_cond = k_injected.chunk(2)
#                 k_sa = torch.cat([k_uncond] * b + [k_cond] * b, dim=0)
#                 k = self.ratio_for_self_att * k_sa + (1. - self.ratio_for_self_att) * k
